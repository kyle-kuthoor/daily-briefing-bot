...
def get_weather():
    ...
    # 5 day forecast (free tier gives 5 days in 3hr intervals)
...
def get_nba_playoffs():
    ...
    url = f"https://api.balldontlie.io/v1/games?per_page=10&start_date={week_ago}&end_date={today}&postseason=true"
    headers = {"Authorization": BALLDONTLIE_API_KEY}
    response = requests.get(url, headers=headers)
    data = response.json()
    games = data['data']
    ...
    for key, s in series.items():
        home_w = s['home_wins']
        visitor_w = s['visitor_wins']
        msg += f"{s['visitor']} vs {s['home']}: {visitor_w}-{home_w}\n"
...
async def send_daily_briefing():
    ...
    try:
        ...
        message += "🏆 F1 standings unavailable\n\n"
